...
void Usage(char *str)
{
	...
	printf ("\t -t for Target Code Optimization \n");
	exit(1);
}
...
main (int argc, char **argv)
{
   ...
   if ((argc-optind) != 1) {
	...
	if(ic_opt){
		optimize_flags=get_optim(optimize_str);
		if(optimize_flags == -1 ){
			Usage(argv[0]);
		}
	}
...
//	 yydebug=1;
   ...
   if (result == 0) {
	...
	if(ic_opt){
		optimize_intermediate_code(optimize_flags,verbose);
	}else{

		/* 
		No optimization ..however, if verbose we print 
		IC.
		*/
		if(verbose ==1 ){
			print_tac();
	...
	if(tc_opt) {
		generate_optimized_target_code();
	}else{
